...
function setMilesToMeters(miles){
  number = parseInt(miles, 10);
  var newValue = 0, validityBit = true, badBit = false;
  if (isNaN(number)) {
    validityBit = false;
    badBit = true;
    newValue = miles + " - not a valid number";
  } else {
    newValue = number;
    newValue = Math.floor(newValue * 1609.34);
  }       
  return {
    valid: validityBit,
    value: newValue, 
